...
describe('T155 characterize: saveWorkoutCore snapshot shape', () => {
  ...
  it('snapshot has version:1 at top level', async () => {
    await saveWorkoutCore(db, buildInput())
    const [workout] = db.select().from(schema.logged_workouts).all()
    const snap = workout.template_snapshot as Record<string, unknown>
    expect(snap.version).toBe(1)
  })

  it('snapshot top-level keys are exactly: version, name, modality, notes, coaching_cues, slots', async () => {
    await saveWorkoutCore(db, buildInput())
    const [workout] = db.select().from(schema.logged_workouts).all()
    const snap = workout.template_snapshot as Record<string, unknown>
    expect(Object.keys(snap).sort()).toEqual(
      ['coaching_cues', 'modality', 'name', 'notes', 'slots', 'version']
    )
  })

  // NOTE: T155 will add week_number to snapshot. Currently absent.
  it('snapshot does NOT contain week_number (pre-T155 baseline)', async () => {
    await saveWorkoutCore(db, buildInput())
    const [workout] = db.select().from(schema.logged_workouts).all()
    const snap = workout.template_snapshot as Record<string, unknown>
    expect(snap).not.toHaveProperty('week_number')
  })

  it('snapshot slot keys are exactly the expected set', async () => {
    await saveWorkoutCore(db, buildInput())
    const [workout] = db.select().from(schema.logged_workouts).all()
    const snap = workout.template_snapshot as { slots: Record<string, unknown>[] }
  ...
  it('snapshot slots are sorted by order ascending', async () => {
    await saveWorkoutCore(db, buildInput())
    const [workout] = db.select().from(schema.logged_workouts).all()
    const snap = workout.template_snapshot as {
  ...
  // NOTE: T155 should make snapshot reflect week overrides. Currently it captures
  // base slot values only — no override awareness.
  it('snapshot captures base slot values (no override awareness)', async () => {
    await saveWorkoutCore(db, buildInput())
    const [workout] = db.select().from(schema.logged_workouts).all()
    const snap = workout.template_snapshot as {
  ...
  // NOTE: saveWorkoutCore takes no weekNumber param currently. T155 will likely add it.
