...
## Files
- `actions.ts` — Server Action entry points (`saveWorkout`, `saveRunningWorkout`, `saveMmaWorkout`, `saveMixedWorkout`), handles cache revalidation. On success, fires `syncCompletion` (Google Calendar checkmark update) as fire-and-forget via `.catch(() => {})`.
