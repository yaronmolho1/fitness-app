...
- `queries.ts` — read queries: `getGoogleCredentials()`, `isGoogleConnected()`, `getEventMapping()`, `getEventsByMesocycle()`, `getAthleteTimezone()`
- `actions.ts` — Server Action: `disconnectGoogle()` — removes credentials, optionally deletes Fitness calendar via API
- `sync.ts` — sync orchestration: `syncMesocycle()` (batch-push projected workouts), `syncScheduleChange()` (diff-based create/delete on schedule edits), `syncCompletion()` (checkmark title update on workout log), `buildEventBody()` (event body builder with modality colors)
