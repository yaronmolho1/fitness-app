...
- `sync.ts` — sync orchestration: `syncMesocycle()` (batch-push projected workouts), `syncScheduleChange()` (diff-based create/delete on schedule edits), `syncCompletion()` (checkmark title update on workout log), `retryFailedSyncs()` (re-attempt all error-status events), `deleteEventsForMesocycle()` (bulk-delete events for a mesocycle), `getCalendarContext()` (shared setup: connection check + calendar API), `buildEventBody()` (event body builder with modality colors)
