...
describe('getTodayWorkout — schedule resolution baseline (T184 characterize)', () => {
  ...
  it('schedule_week_overrides are ignored by current code', async () => {
    // Even with an override that swaps template, current code reads weekly_schedule only
    const meso = seedMesocycle({ status: 'active', start_date: '2026-03-02' })
    const ex = seedExercise('Bench Press')
    ...
    if (results[0].type === 'workout') {
      ...
      expect(results[0].time_slot).toBeNull() // base has no time_slot
    }
  })

  it('schedule_week_overrides null template_id does not cause rest day in current code', async () => {
    // Override sets template_id=NULL (rest override), but current code ignores overrides
    const meso = seedMesocycle({ status: 'active', start_date: '2026-03-02' })
    const ex = seedExercise('Squat')
  ...
  it('override-only entry (no base for period) does not appear in current code', async () => {
    // Override adds a workout to evening, but no base evening entry exists
    const meso = seedMesocycle({ status: 'active', start_date: '2026-03-02' })
    const ex = seedExercise('Bench Press')
    const tmpl1 = seedTemplate(meso.id, 'Morning Push')
