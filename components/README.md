...
- `mesocycle-form.tsx` — `MesocycleForm` create/edit form with name, start date (DatePicker), work weeks, deload toggle, live end date preview; accepts `existingMesocycles` + `currentStatus` props — shows yellow warning for draft overlaps, red error + disabled submit for planned/active overlaps
- `schedule-grid.tsx` — `ScheduleGrid` 7-day weekly schedule grid; assign/replace/remove/edit template per day slot via `assignTemplate`/`removeAssignment`/`updateScheduleEntry` actions; auto-fills duration from `estimated_duration` → `target_duration` → `planned_duration` on template select; supports rotation slots (cycle_length > 1) with badge display, popover summary, and "Edit Rotation"/"Remove" actions; "Add Workout" offers "Assign template" or "Assign rotation" choice; read-only when mesocycle is completed
- `status-badge.tsx` — `StatusBadge` colored pill for mesocycle status (draft/planned/active/completed)
...
- `clone-mesocycle-form.tsx` — `CloneMesocycleForm` clones a mesocycle via `cloneMesocycle` server action; accepts `existingMesocycles` prop for client-side overlap detection (yellow warning, non-blocking since clones start as draft)
...
- `rotation-editor-modal.tsx` — `RotationEditorModal` dialog for editing per-day template rotation cycles; configures cycle length (2–8), assigns a template to each cycle position, auto-fills per-position duration from `estimated_duration` → `target_duration` → `planned_duration`; saves via `assignRotation`; exports `RotationTemplate`, `ExistingRotationPosition` types; props: `mesocycleId`, `dayOfWeek`, `weekType`, `timeSlot`, `duration`, `existingRotation`, `templates`
