...
describe('CloneMesocycleForm', () => {
  ...
  it('pre-fills work_weeks and has_deload from source', () => {
    ...
    const deloadCheckbox = screen.getByLabelText(/deload/i)
    expect(deloadCheckbox).toHaveAttribute('data-state', 'checked')
  })
  ...
  it('pre-fills has_deload=false from source', () => {
    ...
    const deloadCheckbox = screen.getByLabelText(/deload/i)
    expect(deloadCheckbox).toHaveAttribute('data-state', 'unchecked')
  })
